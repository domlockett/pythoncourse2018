...
class Portfolio(object):
    def __init__(self):
        self.cash = 0
        hist= [('Account created: %d-%d-%d %d:%d - $ 0' % (now.year, now.month, now.day, now.hour, now.minute))]
        #port = {"Stock" : {}, "Mutual_Fund" : {}}

    ## Add cash to the portfolio
    def addCash(self,amount):
        self.cash += float(amount)
        hist.append('Deposited $%.f: %d-%d-%d %d:%d - $ %.2f' % (now.year, now.month, now.day, now.hour, now.minute, self.cash) )
        return self.cash
    
    ## Buys stock with the number of shares and the particular stock 
    def buyStock(self,shares, stock):
        self.cash -= (float(amount)*stock.price)
        hist.append = ('Purchased %.2f of %s: %d-%d-%d %d:%d - $ %.2f' % (shares, stock, now.year, now.month, now.day, now.hour, now.minute,self.cash) )
        return self.cash
